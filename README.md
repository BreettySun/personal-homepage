...
- 天气：启动后拉一次 Open-Meteo 的北京实时天气，WMO 代码映射成 晴/阴/雨/雪 四态，请求失败就按当前季节取默认天气；图例里的参数面板或终端可以手动覆盖，覆盖值存在 `localStorage`。面板里的 `density` 滑条调地形线条的疏密（0.4× 到 1.6× 的默认行数），同样作为手动覆盖保存。雨、雪、秋天的落叶、春天晴日的柳絮是粒子（雨是短线段"雨丝"，柳絮往上飘）；任何季节的阴天都有几片流云贴着地形上方横移，浓度随 intensity；晴和阴还靠雾的远近、雾色偏灰程度和线条对比度区分（`src/terrain/atmosphere.ts`）。这些切换全部缓动。
